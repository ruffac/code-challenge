export const codeWarsUserAPI = "https://www.codewars.com/api/v1/users/";
export const codeWarsCollectionsAPI = "https://www.codewars.com/collections";
export const cron = 1 * 24 * 60 * 60 * 1000;

export const MAX_STUDENTS_ON_TABLE = 10;

export const defaultChallengeDuration = 42; // in days
export const defaultChallengeType = "upliftcodecamp";
export const challengeTypes = {
  upliftcodecamp: "upliftcodecamp",
  javascriptbeginnerfriendly: "javascript-beginner-friendly",
};
export const challengesIds = {
  upliftcodecamp: [
    "59441520102eaa25260000bf",
    "5b853229cfde412a470000d0",
    "53b138b3b987275b46000115",
    "56efc695740d30f963000557",
    "52fba66badcd10859f00097e",
    "5420fc9bb5b2c7fd57000004",
    "54e6533c92449cc251001667",
    "6319dba6d6e2160015a842ed",
    "51c8991dee245d7ddf00000e",
    "5727bb0fe81185ae62000ae3",
    "515e188a311df01cba000003",
    "515e188a311df01cba000003",
    "57ee99a16c8df7b02d00045f",
    "521c2db8ddc89b9b7a0000c1",
    "5694cb0ec554589633000036",
    "5703c093022cd1aae90012c9",
    "55a144eff5124e546400005a",
    "56b97b776ffcea598a0006f2",
    "57fe864854685b1c420002e0",
    "563d59dd8e47a5ed220000ba",
  ],
  javascriptbeginnerfriendly: ["55a70521798b14d4750000a4"],
};
